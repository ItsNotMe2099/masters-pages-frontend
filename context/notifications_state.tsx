...
import { INotificationRecord, NotificationUnreadStoreType } from 'types/types'
import { NotificationUnreadType } from 'types/enums'
...
interface IState {
  ...
  getTotalByRolesAndTypes: (role: ProfileRole, types: NotificationType[]) => number
}
...
const defaultValue: IState = {
  ...
  getTotalByRolesAndTypes: (role, types) => 0
}
...
export function NotificationWrapper(props: Props) {
  ...
  const byTypes = useMemo(() => {
    const val: ByTypes = {}
    for (const item of stats.items.filter(i => !i.role)) {
      val[item.type] = item.count
    }
    return val
  }, [stats])
  const byRoles = useMemo(() => {
    const val: ByTypes = {}
    for (const item of stats.items.filter(i => i.role)) {
      val[item.role] = item.count
    }
    return val
  }, [stats])

  const getTotalByType = (type: NotificationType) => {
    return stats.items.filter(i => !i.role).find((i) => i.type === type)?.count || 0
  }
  const getTotalByTypes = (types: NotificationType[]) => {
    return stats.items.filter(i => !i.role).filter((i) => types.includes(i.type)).reduce((a, b) => a + b.count, 0) || 0
  }
  const getTotalByRolesAndTypes = (role: ProfileRole, types: NotificationType[]) => {
    return stats.items.filter(i => i.role === role).filter((i) => types.includes(i.type)).reduce((a, b) => a + b.count, 0) || 0
  }
