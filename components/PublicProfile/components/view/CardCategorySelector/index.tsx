 ...
 console.log("Categories", categories);
  ...
  return (
    <div className={styles.root}>
      ...
      <DropDownCategory options={categories.map(category => ({value: category.category.id, label: getCategoryTranslation(category.category).name}))} item={(item) => <div>{item?.label}</div>} onChange={handleChangeCategory} value={props.category?.category.id} />
      <div className={styles.separator}/>
      </div>
        {props.category?.skills.map(skill => skill.subCategory ? <Tab isActive={props.subCategory?.subCategory.id === skill.subCategory.id} title={getCategoryTranslation(skill.subCategory).name} onClick={() => handleChangeSubCategory(skill)}/> : null)}
