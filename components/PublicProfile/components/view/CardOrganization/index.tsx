    ...
    <Card className={styles.root} toolbar={isEdit ? [<FormActionButton type={'edit'} title={showForm ? t('cancel') : t('task.edit')} onClick={handleEditClick} />] : []}>
      ...
      <div className={styles.allStats}>
        ...
        <div className={styles.followers}>
          {!isEdit && (isSubscribed ? <div className={styles.subscribed}>{t('cardProfile.subscribed')}</div> : <Button className={styles.actionFollow} color={'green'} disabled={recommendationLoading} href={!profile ? '/registration/user' : null} onClick={() => profile ? handleSubscribe() : null}>{t('personalArea.profile.subscribe')}</Button>)}
          {!isEdit && <Button className={styles.actionFollow} color={'green'} href={profile ? `/Chat/dialog/${organization.corporateProfile.id}` : '/registration/user'}>{t('personalArea.profile.sendMessage')}</Button>}
          {!isEdit && <Button className={styles.actionFollowLast} color={'green'} href={profile ? `${Routes.profile(organization.corporateProfile)}/news` : '/registration/user'}>{t('news')}</Button>}
        </div>
      </div>
    </Card>
  )
