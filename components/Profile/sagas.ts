...
import {
  ...
  createProfile,
  deleteProfile,
  deleteProfileRequest, hideProfileForm, updateProfile, updateProfileAvatar, updateProfileByForm
} from './actions'
import {IRequestData, IResponse} from 'types'
import Router from 'next/router'
import cookie from 'js-cookie'
import {confirmChangeData} from '../Modal/actions'
  ...
  yield takeLatest(ActionTypes.CHANGE_ROLE,
    function* (action: ActionType<typeof changeRole>) {
      ...
      if (!res.err && res.data && res.data.id) {
        cookie.set(CookiesType.profileRole, action.payload.role, {expires: 60 * 60* 24 * 365})
        meRedirect()
      } else {
